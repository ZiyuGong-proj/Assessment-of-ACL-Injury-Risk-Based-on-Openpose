...
def run_parser_plot_out():
    fig, ax = plt.subplots(num="D1：踝肩测试")
    ln, = ax.plot([], [], lw=2)
  ...
  #              print(p)
   #             x += 1
    ...
    def data_front():
        x = 0
        while 1:
            for i in OpenposeJsonParser_front().stream_update_point_change_data_in_the_dir3("/home/jasonccf/openpose/examples/media_out",
                                                                    sum=True):
                p.append(Index_score.Return_index_5(i))
                print(p)
                x += 1
                yield  x,i
