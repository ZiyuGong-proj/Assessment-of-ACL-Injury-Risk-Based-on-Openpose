...
def run_parser_plot_out():
    fig, ax = plt.subplots(num="D1:Ankle-knee test")
    ln, = ax.plot([], [], lw=2)
    ax.set_ylim(0, 100)  # 设置scale
    ax.set_xlim(0, 50)
    ax.grid()
    xdata, ydata = [], []
  ...
  #              print(p)
   #             x += 1
    ...
    def data_front():
        x = 0
        while 1:
            for i in OpenposeJsonParser_front().stream_update_point_change_data_in_the_dir2("/home/jasonccf/openpose/examples/media_out",
                                                                    sum=True):
                p.append(Index_score.Return_index_4(i))
                print(p)
                x += 1
                yield  x,i
