...
def run_parser_plot_out():
    fig, ax = plt.subplots(num="A2：躯腿测试")
    ln, = ax.plot([], [], lw=2)
    ...
    def data_sideways():
        ...
        while 1:
            for i in OpenposeJsonParser_sideways().stream_update_point_change_data_in_the_dir_2("/home/jasonccf/openpose/examples/media_out",
                                                                    sum=True):

                p.append(Index_score.Return_index_2(i))
                print(p)
                x += 1
                yield x,i
